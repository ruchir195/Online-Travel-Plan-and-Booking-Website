...
const register = async (req, res) => {
    try {
        ...
        const userData = await User.create({ username: username, email: email, phone: phone, password: secPass });
        ...
        res.status(200).json({ msg: userData, tocken: await authTocken, userId: userData._id.toString(), userData })
        ...
        if (userExists) {
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    res.status(200).json({ msg: "Email Sent Succesfully", userid, otp: otp });
...
const otpVerification = (req, res) => {
    try {
        ...
        res.status(200).json({ msg: "Valid OTP", userotp: userotp, genotp: genotp });
    } catch (error) {
...
const confirmPassword = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $set: { password: secPass } }, { new: true });

        res.status(200).json({ msg: "Password reset successfully done.", user: user, userId: userId });
...
module.exports = { home, register, login, googleCheck, googleLogin, forgotPassword, otpVerification, confirmPassword };